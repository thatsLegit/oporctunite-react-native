...
const ProlapsusUterinEtRectal = props => {
    ...
    const noteRectal = Math.round((((evaluation.nbTruies-count3+count4) / globalCount) * 10 + Number.EPSILON) * 10) / 10;

    const validationHandler = async () => {
        await dispatch(testActions.ajouterTest(note, evaluation.nomEvaluation));
        await dispatch(testActions.ajouterTest(noteRectal, evaluation2.nomEvaluation));
        if (Vtype == 'valider') {
